...
// animations keys
fileprivate let kMKShimmerSlideAnimationKey = "slide"
fileprivate let kMKFadeAnimationKey = "fade"
fileprivate let kMKEndFadeAnimationKey = "fade-end"
...
fileprivate func shimmer_slide_animation(duration: TimeInterval,direction: MKShimmerDirection) -> CABasicAnimation {
...
fileprivate func shimmer_slide_repeat(a: CAAnimation, duration:TimeInterval,direction: MKShimmerDirection) -> CAAnimation {
...
class MKShimmerMaskLayer: CAGradientLayer {
...
class MKShimmerLayer: CALayer {
    var maskLayer: MKShimmerMaskLayer?
    ...
    var shimmeringDirection: MKShimmerDirection {
        didSet{
    ...
    override func layoutSublayers() {
        super.layoutSublayers()
        let r = self.bounds
        contentLayer?.anchorPoint = CGPoint(x: 0.5, y: 1)
    ...
    func updateShimmering() {
        ...
        if !shimmering {
            ...
            if let slideAnimation = maskLayer?.animation(forKey: kMKShimmerSlideAnimationKey) {
                ...
                maskLayer?.add(finishAnim, forKey: kMKShimmerSlideAnimationKey)
            }
            ...
            fadeInAnimation.setValue(true, forKey: kMKFadeAnimationKey)
            maskLayer?.fadeLayer.add(fadeInAnimation, forKey: kMKFadeAnimationKey)
            ...
            if shimmeringBeginFadeDuration > 0 && !disableActions {
                ...
                maskLayer?.fadeLayer.add(fadeOutAnimation!, forKey: kMKFadeAnimationKey)
            } else {
            ...
            var slideAnim = maskLayer?.animation(forKey: kMKShimmerSlideAnimationKey)
            guard slideAnim == nil else {
                maskLayer?.add(shimmer_slide_repeat(a: slideAnim!, duration: TimeInterval(animationDuration), direction: shimmeringDirection), forKey: kMKShimmerSlideAnimationKey)
                return
            }
            ...
            maskLayer?.add(slideAnim!, forKey: kMKShimmerSlideAnimationKey)
        }
    }
}

extension MKShimmerLayer: CALayerDelegate {
    func action(for layer: CALayer, forKey event: String) -> CAAction? {
        return nil
    }
}

extension MKShimmerLayer: CAAnimationDelegate {
    func animationDidStop(_ anim: CAAnimation, finished flag: Bool) {
        guard let key = anim.value(forKey: kMKEndFadeAnimationKey) as? Bool  else { return }
        if flag && key {
            maskLayer?.fadeLayer.removeAnimation(forKey: kMKFadeAnimationKey)
            self.clearMask()
        }
    }
}
