...
struct DBPath {
    let filePath = NSSearchPathForDirectoriesInDomains(FileManager.SearchPathDirectory.documentDirectory, FileManager.SearchPathDomainMask.userDomainMask, true).last! + "/sample.db"

}

class XSDBManager: NSObject {
    static let share = XSDBManager()
    
    var dataBasePath = URL(fileURLWithPath: DBPath().filePath)
    ...
    func createDB() -> Database {
        debugPrint("数据库地址=="+dataBasePath.absoluteString)
